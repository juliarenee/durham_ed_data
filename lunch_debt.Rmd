...
# Calculate Title 1 banding for 2017-18
fps = bands$alloc_per_student %>% unique()
limits = c()
for(i in 1:length(fps)) {
  limits[i] = max(bands$pct_low_income[bands$alloc_per_student==fps[i]])
}

# Compare low-income percentage data
df.trimmed = df %>%
  filter(year == "2017-18",
         eligible == 1,
         participating == 0) %>%
...
# TODO: Pull dates for Title I and ISP data don't match up, which means FRP/low-income percentages differ

df.trimmed$isp_mult = df.trimmed$isp*1.6


dat = left_join(df.trimmed, bands, )
```
