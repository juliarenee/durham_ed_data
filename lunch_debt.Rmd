...
df = df %>% select(c("school_no","school_name","year","pct_free_reduced","cep","grade"))
...
# change col name
colnames(dem)[which(colnames(df)=="state_school_id")] = "school_no"
...
missingYears = df$school_no[is.na(df$pct_needy)] %>% unique()

t = df %>%
  filter(school_no %in% missingYears) %>%
  select(school_no, year, pct_needy) %>%
