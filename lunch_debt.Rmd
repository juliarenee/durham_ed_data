...
df$school_name[is.na(df$adm)] %>% unique()
```
...
```
Most schools have less than $5 of lunch debt per student, which is reasonable. Let's try dig into schools with extremely high debt, large fluctuations in debt, and the effects of CEP status.
...
FOR NEXT WEEK:
- fill missing adm/pct needy data
