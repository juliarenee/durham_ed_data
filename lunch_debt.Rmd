...
# See how banding changes
bands19$band_before = bands19$pct_low_income %>%
  cut(., breaks = limits)
bands19$band_after = bands19$pct_w_cep %>%
  cut(., breaks = limits)

# Approximate funding changes
bands19$alloc_w_cep = bands19$alloc_per_student*(bands19$n_low_income/bands19$pct_low_income)*bands19$pct_w_cep
bands19$alloc_diff = bands19$alloc_w_cep - bands19$total_alloc 

bands19[bands19$band_before != bands19$band_after,] %>%
  select(school_name, pct_low_income, pct_w_cep, band_before, band_after, alloc_diff) %>%
  arrange(-alloc_diff)
```
