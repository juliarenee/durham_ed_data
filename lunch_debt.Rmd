...
## TODO: recalculate FRP percentages for schools with CEP in years where < 20% for reduced (set = 0)
...
# Recalculate bands based on percent low-income, ignoring rank
bands19$isp_band = bands19$pct_w_cep %>%
  cut(., breaks = limits)

# Approximate funding changes based on percent low-income, ignoring rank
bands19$alloc_band = unlist(aps[bands19$isp_band])*(bands19$n_low_income/bands19$pct_low_income)*bands19$pct_w_cep
bands19$alloc_band_diff = bands19$alloc_band - bands19$total_alloc 

bands19[bands19$orig_band != bands19$isp_band,] %>%
  select(school_name, pct_low_income, pct_w_cep, orig_band, isp_band, alloc_band_diff) %>%
  arrange(-alloc_band_diff)

# Recalculate bands based only on rank
bands19 = bands19 %>%
  arrange(-rank_after)
bands19$rank_band = 0
bands19$rank_band[1:10] = "(0.85,1]"
bands19$rank_band[11:15] = "(0.74,0.85]"
bands19$rank_band[16:24] = "(0.62,0.74]"
bands19$rank_band[25:length(bands19$rank_band)] = "(0,0.62]"

# Approximate funding changes based only on rank
bands19$alloc_rank = unlist(aps[bands19$rank_band])*(bands19$n_low_income/bands19$pct_low_income)*bands19$pct_w_cep
bands19$alloc_rank_diff = bands19$alloc_rank - bands19$total_alloc 

bands19[bands19$orig_band != bands19$rank_band,] %>%
  select(school_name, pct_low_income, pct_w_cep, orig_band, rank_band, alloc_rank_diff) %>%
  arrange(-alloc_rank_diff)
```
