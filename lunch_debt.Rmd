...
km.train = df %>%
  filter(ts == 2014) %>%
  select(-c(adm, grade, ts, predict, predict_ind)) %>%
  select(unpaid, pct_free_reduced, pct_minority, debt_per_student) %>%
...
# run k-means
# 2 clusters, hopefully high and low need schools
km.out = kmeans(km.train, centers = 2, nstart = 20)

km.out$cluster
km.out$betweenss

#TODO: PCA reduces data to one dimension. Maybe that can be our measure of need?
```
