...
# get 2016-17, 2017-18 demographic data from pdf
# only need second page
dem17 = pdf_text("./data/durham_dem_1617.pdf")[2]
dem18.1 = pdf_text("./data/durham_dem_1718.pdf")[2]
dem18.2 = pdf_text("./data/durham_dem_1718.pdf")[3]

# FUNCTION: to extract data from PDFs
clean_table = function(txt, start) {
  v = str_split(txt, "\n", simplify = TRUE) # split into rows
  v = v[start:length(v)] # ignore messy headers
  split = str_split_fixed(v, "320", 2) # split on lea
  lea = split[,1] # get school names
  
  # get vals as matrix
  dat = lapply(split[,2], function(x) unlist(strsplit(x, "[[:blank:]]+"))) %>% 
    unlist() %>%
    matrix(., ncol = 32, byrow = TRUE)
  
  dat = dat[,c(1,30,31,seq(from = 4, to = 28, by = 4))] # only keep totals, school number
  dat = cbind(lea, dat) %>% as.data.frame(stringsAsFactors = FALSE) # convert to data frame
  colnames(dat) = c("school_name",
                    "state_school_id",
                    "male",
                    "female",
                    "american_indian_alaska_native",
                    "asian_or_asian_pacific_islander",
                    "hispanic",
                    "black",
                    "hawaiian_nat_pacific_isl",
                    "white",
                    "two_or_more_races")
  return(dat)
}
...
# clean tables
dem17 = clean_table(dem17, start = 4)
dem18.1 = clean_table(dem18.1, start = 5)
dem18.2 = clean_table(dem18.2, start = 3)
dem18 = rbind(dem18.1, dem18.2) # bind all 2017-18 data
...
# vars to numeric
dem17[,c(3:11)] = lapply(dem17[,c(3:11)], as.numeric)
dem18[,c(3:11)] = lapply(dem18[,c(3:11)], as.numeric)

dem = plyr::rbind.fill(dem, dem17, dem18)
...
## Merging Checks
We're pulling data from lots of different sources. Let's see where there may be disparities.

```{r}
# merging checks
tot.students = df %>%
  filter(male+female != adm) %>%
  group_by(school_no, year) %>%
  summarise(mf = male+female,
            adm = adm)
...
dim(df)
```
Most of our student counts don't match up by the simplest measure: comparing the total ADM, retrieved from free/reduced lunch data, to the total number of male and female students. According to KC Elander at NC DPI, this is because data is pulled at different points in time, from different government agencies and for different requests.
...
```
For the most part, aside from missing data, it seems like schools with no debt are CEP schools. We can't get CEP status starting in 2010-11, since those were pilot years. Also something to note: There are a decent number of NA's. After checking some schools out, we don't have data for them from any dataset outside of school lunch debt. But some of them are still running? I've emailed KC about this.
...
Anyway, I'll stick to the log transformation and continue our EDA.

### Debt and Demographics
```{r, fig.width=12, fig.height=4, fig.align="center"}
# scatterplot of debt vs. pct needy
ggplot(df, aes(pct_needy, log_dps)) +
  geom_point() +
  xlab("Percent Needy") +
  ylab("log(debt-per-student)") +
  geom_smooth(method = "lm")

# scatterplot of debt vs. pct black
g1 = ggplot(df, aes(black/adm, log_dps)) +
  geom_point() +
  xlab("Percent Black") +
  ylab("log(debt-per-student)") +
  geom_smooth(method = "lm")
...
g2 = ggplot(df, aes(hispanic/adm, log_dps)) +
  geom_point() +
...
g3 = ggplot(df, aes(white/adm, log_dps)) +
  geom_point() +
  xlab("Percent White") +
  ylab("log(debt-per-student)") +
  geom_smooth(method = "lm")
