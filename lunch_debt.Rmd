...
to.rm = which(table(locales$school_no) > 1) # schools that have more than 1 local
#for(sch in to.rm) {
#  keep = 
#  print(keep)
#}
...
# filter to schools without cep status
# includes schools that gained cep status for non-cep years
df.non.cep = df[df$cep == 0,]

# predict debt by demographics
lm.dem = lm(debt_per_student ~ pct_white + pct_black + pct_hispanic, data = df.non.cep)
summary(lm.dem)
...
lm.need = lm(debt_per_student ~ pct_free_reduced, data = df.non.cep)
summary(lm.need)
...
ggplot(df, aes(urban_centric_locale, debt_per_student)) +
  geom_boxplot(aes(col = cep_long))
```
...
FOR NEXT WEEK:
- schools with CEP 2017-18 have over 100% needy?
