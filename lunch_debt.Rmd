...
grades = unique(df$grade) %>% sort(decreasing = TRUE)
names(grades) = grades
grades[1:2] = "Elementary"
grades[c(3:4,8)] = "High"
grades[c(5:6)] = "Middle"
grades[7] = "Middle and High"

# add schcool level based on clean grades
g$grade_clean = grades[g$grade]

# ignore 342 for now
g$grade_clean[g$school_no == "342"] = NA

# only unique vals
g = g %>% select(school_no, grade_clean) %>% unique()

# merge with data
df = left_join(df, g, by = "school_no")
