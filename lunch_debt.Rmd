...
df = df[-which(colnames(df)=="school_name.y")]
...
```{r}
year_seq = seq(0,7)
names(year_seq) = unique(df$year)
df$year_seq = year_seq[df$year]

df$adm = df$adm %>% as.numeric()

ggplot(df, aes(year_seq, unpaid, col = school_no)) +
  geom_point() +
  geom_line() +
  scale_y_continuous(labels = scales::comma)

ggplot(df, aes(year_seq, unpaid/adm, col = school_no)) +
  geom_point() +
  geom_line() +
  scale_y_continuous(labels = scales::comma)
```
