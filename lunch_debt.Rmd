...
clean_table = function(txt, start) {
  ...
  dat = cbind(lea, dat) %>% as.data.frame() # convert to data frame
...
```
Alright, much better! It's bimodal, but what we're seeing in the first node (centered around roughly -4) are years where a school had no debt. These are likely CEP schools.
...
# boxplot by grades
b1 = boxplot(df$debt_per_student[df$elementary==1])
b2 = boxplot(df$debt_per_student[df$middle==1])
b3 = boxplot(df$debt_per_student[df$high==1])
